...
**The automation cannot start a named agent.** `orca automations create` takes `--provider claude` and has no `--agent` or `--model`, so the per-role definition that Story 0.1 verified — the one thing the runtime does enforce — does not apply to Scotty's scheduled session. Two consequences, and both are handled rather than tolerated: his `model: sonnet` is pinned instead by a committed `.claude/settings.json`, which sets the default for sessions started without an agent and leaves `claude --agent <lead>` alone; and his mandate is loaded by the prompt telling him to read his own definition, `scripts/automation/scotty-prompt.md` holding only that instruction and never restating the role.
...
- `orca automations` supports `--precheck`, `--workspace-mode`, `--base-branch`, `--missed-run-grace-minutes`, `--timezone`, `--reuse-session`, and cron/RRULE triggers. **It has no `--agent` and no `--model`**, so a scheduled session cannot be started as one of the six roles and does not inherit that role's model or tool classes. `orca orchestration worker-start` has both flags; automations do not. The precheck is stored with a `timeoutSeconds` of 60 by default — ours runs in about three — and `--timezone` defaults to the machine's, recorded as `Europe/Zurich`, which a fixed `*/12` interval never consults.
...
- **`skipReason` is `null` even on a precheck skip.** The run record's `status` is the only field that distinguishes `dispatched` from `skipped_precheck`. Any watchdog keys on `status`, never on `skipReason` — **and `status` alone is not enough either**: `dispatched` is written when Orca starts a session, not when it does anything, so a session that received no prompt records exactly what one that merged a PR records. The watchdog keys on evidence the team moved.
