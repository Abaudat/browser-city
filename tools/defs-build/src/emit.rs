...
// no direct type import needed beyond what `Defs` already carries.
...
fn fmt_role_rust(role: &Option<RoleDef>) -> String {
    match role {
        None => "None".to_string(),
        Some(r) => format!("Some(RoleDef {{ layers: &{} }})", fmt_u32_slice(&r.layers)),
    }
}

fn fmt_neighbour_term_rust(term: &NeighbourTermDef) -> String {
    format!(
        "crate::rules::NeighbourTerm {{ direction: {}, tag: {}, present: {} }}",
        fmt_direction_rust(term.direction),
        term.tag,
        term.present
    )
}

fn fmt_alternatives_rust(alternatives: &[Vec<NeighbourTermDef>]) -> String {
    let alts: Vec<String> = alternatives
        .iter()
        .map(|alt| {
            let terms: Vec<String> = alt.iter().map(fmt_neighbour_term_rust).collect();
            format!("&[{}]", terms.join(", "))
        })
        .collect();
    format!("&[{}]", alts.join(", "))
...
fn fmt_rule_kind_rust(kind: &RuleKindDef) -> String {
    match kind {
        ...
        RuleKindDef::Adjacency {
            ...
            "crate::rules::RuleKind::Adjacency {{ a: {a}, relation: {}, alternatives: {} }}",
            fmt_adjacency_relation_rust(*relation),
            fmt_alternatives_rust(alternatives)
        ),
...
fn fmt_role_json(role: &Option<RoleDef>) -> String {
    match role {
        None => "null".to_string(),
        Some(r) => format!("{{ \"layers\": {} }}", fmt_u32_slice(&r.layers)),
    }
}
...
pub fn emit_json(
    ...
    out.push_str(&format!(
        "  \"character_composite_pages\": {CHARACTER_COMPOSITE_PAGES},\n"
    ));
    ...
    // Story 3.4: building types are server-only generation content, the
    // same reason `rules` (RULES) never reaches `defs.json` either --
    // the client's own `parseDefs` allow-list (`client/src/defs/
    // parse.ts`) is never extended for a kind it does not yet consume.
    ...
    // Story 2.7: every part's own packed `atlas` rect -- JSON-only, like
...
pub fn emit_id_manifest(defs: &Defs) -> String {
    ...
    let mut building_types = defs.building_types.clone();
    building_types.sort_by_key(|b| b.id);
...
mod tests {
    ...
    /// Story 2.7: every appearance part's own `atlas` rect, JSON-only, and
    /// the `character_composite_pages` constant (Tim's direction).
    #[test]
    fn emit_json_renders_each_character_parts_atlas_rect_and_the_composite_page_constant() {
        let out = emit_json(
            &sample(),
            "v1",
            &sample_atlas_pages(),
        ...
        assert_eq!(
            out.matches("\"atlas\": { \"h\": 32, \"page\": 1, \"w\": 16, \"x\": 2, \"y\": 2 }")
                .count(),
            5,
            "{out}"
        );
        assert!(out.contains("\"character_composite_pages\": 2"));
    }
    ...
    #[test]
    #[should_panic(expected = "has no packed atlas rect")]
    fn emit_json_panics_if_a_character_part_is_missing_its_atlas_rect() {
        let _ = emit_json(
            &sample(),
            "v1",
            &sample_atlas_pages(),
