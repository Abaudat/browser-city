...
# therefore be reached by absolute path -- and every one of those paths must
# be *derived*, because a script that names one user's home stops working the
# first time the repo moves, and it stops by exiting non-zero, which reads as
