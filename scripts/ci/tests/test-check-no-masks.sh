...
d="$(plant 'const overlay = new Sprite({ texture, blendMode: "color-dodge" });')"
check "an advanced blend mode named in an object-literal option ({ blendMode: \"color-dodge\" })" 1 bash "$CHECK" "$d"

d="$(plant 'const view = { blendMode: "screen" };')"
check "a basic blend mode named in an object-literal option is never a false positive" 0 bash "$CHECK" "$d"
