...
| A connect error at boot and a drop after a successful connect both surface as `ConnectionStatus` `"disconnected"` (`net/connection.ts`'s plain string union, never an SDK type), and the connection notice shows for either, debounced so a brief blip never flickers it; the notice never takes focus, never suspends the keyboard, never uses a native dialog, and is announced with `role="status"`/`aria-live`; reconnection is out of scope for this story, so once shown the notice stays up | covered | `client/src/net/connection.ts`, `client/src/ui/connection-notice.ts`, `client/tests/unit/connection.test.ts`, `client/tests/unit/ui/connection-notice.test.ts`, `client/tests/e2e/connection-notice.spec.ts` |
| NFR22: no HUD with net worth, balances, counters or objective markers -- the story 1.1 ping indicator (`render/bootstrap.ts`, a fixed-position DOM readout with no requirement behind it) is deleted, and the production bundle is checked to never contain it again | covered | `scripts/ci/check-pages-bundle.sh`, `scripts/ci/check-no-canvas-ui.sh` |
