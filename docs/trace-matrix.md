...
| The restored `auto_inc` sequence matches the live sequence position of the source database at export time, not merely the maximum id present in the restored data -- these differ if a row was ever deleted off the tail of a table before export | deferred | a source database whose sequence keeps advancing after an export is taken and is later compared or merged against the restore; today's restore only claims to reach the restored data's own maximum id, tested directly (`docs/spikes/1.4-backup-restore.md`'s tail-deletion finding) |
