...
| The demo player movement is resolved by `world/movement.ts` swept-AABB `step` against real fixture colliders, and the scene only re-sorts when the player own quantised sort-key position actually changes (story 1.8, replacing the old bounds-clamped `player-step.ts`) | covered | `client/tests/unit/world/movement.test.ts`, `client/tests/unit/demo/drawables.test.ts` |
...
| Walking speed is a named constant derived from `movement.walk_speed_millicells_per_s`, never a literal scattered through movement code; crossing a 40-cell viewport takes ~18s within tolerance across jittered frame deltas (FR137) | covered | `client/tests/unit/world/no-speed-literal.test.ts`, `client/tests/unit/world/movement.test.ts` |
| Client movement code under `world/**` never imports `pixi.js`, `../net/*` (except `net/bindings` types, type-only) or `../demo/*` -- structurally, not just by convention | covered | `client/biome.json`'s `noRestrictedImports` override, `client/tests/unit/world/import-boundaries.test.ts` |
...
| Holding a direction key moves the avatar immediately and continuously with no round trip (FR137); it comes to rest against a real collider and never pauses at a collider-less prop | covered | `client/tests/e2e/movement.spec.ts` |
