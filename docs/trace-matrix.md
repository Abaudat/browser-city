...
| `inv_collision_overlay_shows_exactly_the_colliders` | For any placed world and any viewport, the collision overlay's rects are exactly what the live collision grid reports there -- an independent, from-scratch rebuild over the same window -- plus, and only plus, the objects that declare no collider at all, each at its own true sub-cell position (FR165) | covered | `inv_collision_overlay_shows_exactly_the_colliders` | 1.12 |
...
| An overlay reads the live collision grid the movement resolver reads, never a second expansion of `defs/`; its cost is bounded by the viewport rather than by how many objects the world holds; and with every overlay disabled it does no work at all, so NFR2's frame budget is untouched | covered | `client/src/debug/world-view.ts`, `client/tests/unit/debug/collision-rects.test.ts` -- the live-grid and counting-wrapper tests, `client/tests/unit/debug/overlays.test.ts` -- the zero-reads-while-disabled test |
