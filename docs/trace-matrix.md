...
| The overlay tracks its source every frame it is alive, never a snapshot frozen at hover start: position, scale, anchor, texture and visibility are all mirrored, and a source that goes invisible mid-hover loses its overlay in the same frame; a hammered sequence of hover/un-hover cycles, strength changes and a visibility change interleaved ends at the same container child count before and after the scene handle's own `destroy()`; `setHighlight` with an unchanged id is a genuine no-op -- no teardown, no rebuild, no callback (FR173, NFR2) | covered | `client/src/render/pixi-highlight.ts` -- `HighlightApplier`, `client/tests/unit/render/pixi-highlight.test.ts` |
| The mark never survives its context: opening the options menu (a DOM panel drawn over the canvas), a window blur or the tab hiding all clear the mark and the cursor even with the mouse held perfectly still, and resuming re-lights a still-hovered, still-in-reach object without the mouse moving again (FR173) | covered | `client/src/input/pointer.ts` -- `suspend`/`resume`, `client/tests/unit/input/pointer.test.ts`, `client/src/main.ts` -- the options menu's own `onOpenChange` wiring |
