...
| A full restore has been performed and verified against Maincloud (AC4), including a real, >=100,000-id auto_inc gap-fill under the reducer execution limits Maincloud itself imposes, with its wall time measured, not merely local timings extrapolated | covered | `.github/workflows/backup.yml`'s `rehearsal` job (carves that gap with `scripts/ops/seed-id-gaps.sh` and logs the restore's own wall time) -- run with `rehearsal: true` once the credential this story provisions exists |
...
| Requirement | Status | Guard |
| --- | --- | --- |
| A build under the real Pages base (`/browser-city/`) never references an absolute `/assets/...` path, in `index.html` or baked into the emitted JS/CSS -- a misconfigured base fails the build, not the deploy | covered | `scripts/ci/check-pages-bundle.sh`, run by `ci.yml`'s `client-build` job against a production-style build and by `deploy.yml`'s `deploy-client` job against the real one |
| The production URI actually reaches the build: the `ws://127.0.0.1` local-dev fallback (`client/src/net/config.ts`) never ships in a Pages build | covered | `scripts/ci/check-pages-bundle.sh` (same guard and callers as the row above) |
| The deployed bundle serves the exact commit `deploy.yml` published, not a previous one Pages propagation lag would otherwise let a smoke check pass against | covered | `.github/workflows/deploy.yml` -- the `deploy-client` job's own `<meta name="bc-build">` stamp, and the `smoke` job's bounded poll for it before the spec below ever runs |
| The deployed game actually reaches player-controllable: it loads with no failed request and no console/page error, its WebSocket dials the configured Maincloud URI and database, and its initial subscription applies | covered | `client/tests/e2e/deploy-smoke.spec.ts` (the `deploy-smoke` Playwright project) -- run by `deploy.yml`'s `smoke` job against the live URL, and rehearsed by `ci.yml`'s `e2e` job against a production-base build under `/browser-city/` backed by a disposable local SpacetimeDB |
| A publish never carries a destructive flag (`--delete-data`/`-c`/`--clear-database`/`--break-clients`), and the job that publishes always `needs:` the job that backs up first (NFR39) | covered | `scripts/ci/check-deploy-workflow.sh`, run directly against `deploy.yml` by `ci.yml`'s `scripts-tests` job, with its own fixture tests under `scripts/ci/tests` |
