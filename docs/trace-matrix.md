...
| `inv_outfit_follows_occupation` | The generated civilian outfit is always drawn from the civilian pool, and resolving a profession's uniform override always yields that profession's declared role_only outfit/accessory, for any profession in `defs/professions` that has one (FR62) | covered | `inv_outfit_follows_occupation` | 1.10 |
