...
| Holding a direction key moves the avatar client-side with no round trip (FR137); it comes to rest against a real collider and never pauses at a collider-less prop | covered | `client/tests/e2e/test-street.spec.ts` (which replaced `movement.spec.ts`) |
