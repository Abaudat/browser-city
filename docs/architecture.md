...
A public anonymous view, `module_version` (`server/src/version.rs`, no
table, no reducer, no write on `client_connected`), publishes exactly one
row: `defs_version` (`server/sim/src/generated/defs.rs`'s own
`DEFS_VERSION`) and `protocol_version` -- a second, independent SHA-256,
over every git-tracked file under `client/src/net/bindings/` (the
module's complete client-visible surface), sorted by path, LF-normalised,
truncated to 16 hex characters, generated by `scripts/gen-protocol-
version.sh` into `server/src/generated/protocol_version.rs` and
`client/src/net/protocol-version.ts` (never `tools/defs-build`, which
reads only `defs/`). A view over constants re-evaluates on every read, so
a hot-swap republish is visible with no deploy step and nothing to
migrate (NFR33).

`net/connection.ts` subscribes to `module_version` in the same
`subscribe([...])` call that already carries `demo_ping` -- one subscribe
message, no extra round trip on the match path -- and hands the rest of
the client a plain `{ defsVersion, protocolVersion }`. `boot/handshake.ts`
compares it, by strict equality, against the client's own pair (the
already-fetched `defs.json`'s `defsVersion` plus the compiled-in
`protocol-version.ts` constant): both equal is `proceed`; only
`defsVersion` differs is `refetch-defs` (`defs/load.ts`'s `fetchDefs`,
cache-busted with the server's own version, bounded-retried on a
still-stale result and then degraded rather than looped, NFR42);
`protocolVersion` differs is `reload` -- guarded to once per server
version via a `sessionStorage` key, `updating` on a repeat. `boot/boot-
gate.ts` is the only place these run, and `test-street/scene.ts`'s
`MountStreetSceneOptions.defs` is typed `VerifiedDefs`
(`boot/handshake.ts`), producible only by that gate -- the scene is
structurally unmountable with a defs set the client knows to be stale. An
unreachable connection is "unknown", not "known stale": the gate mounts
the already-fetched defs rather than blocking forever.
