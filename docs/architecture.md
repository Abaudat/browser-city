...
- `server/schema.snapshot.json` (generated by `bounds`'s
  ...
  `scripts/ci/check-schema-additive.sh` diffs it against the PR's merge
  base and fails on a moved primary key, a moved unique constraint, a
  changed scheduled status, a removed/retyped table or column, or a
  column appended without `#[default(...)]` or `#[auto_inc]`.
