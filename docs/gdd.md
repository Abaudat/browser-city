...
| **A4** | **[ASSUMPTION]** The one-second boot is achievable against MMO asset streaming | **High.** The hardest technical constraint in the project, and the brief records that it has not been designed against | **Answered 2026-09-15 by B3 (story 1.14, `docs/spikes/1.14-boot-budget.md`), and the answer is no -- not against today's boot path.** Every measured network/CPU profile misses the 1000 ms budget by 2.6x-5.6x (p75 2.6-5.6s). The dominant cost, on every profile, is fetching and decoding the street crowd's character sheets with no atlas (105 individual image requests, 75-84% of total boot time by itself); the second, D4's ~28k-row subscription decode, misses its own pre-registered 150 ms revisit trigger by ~30x (4.5s at p95, throttled). Recorded against R5/D6 in `docs/architecture.md`'s "Boot budget" section, both reopened. Not a blocker for the story itself (the deliverable was the measurement), but a real, structural risk for whichever story next builds the real boot path (FR144-146) |
