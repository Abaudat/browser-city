...
| **A4** | **[ASSUMPTION]** The one-second boot is achievable against MMO asset streaming | **High.** The hardest technical constraint in the project, and the brief records that it has not been designed against | **Answered 2026-09-15 by B3 (story 1.14, `docs/spikes/1.14-boot-budget.md`), and the answer is a qualified no.** Every measured network/CPU profile misses the 1000 ms budget (p75 1.2-4.2s, worst case 4.22x over, pessimistic/throttled). The dominant cost, on every profile, is fetching *and decoding* the street crowd's character sheets with no atlas (105 individual image requests, measured over the same HTTP/2 protocol GitHub Pages serves in production). D6 (the arithmetic boot-budget split) is reopened by this. D4's own ~28k-row subscription-decode risk is **not** what is driving the miss: cycle 1's own reading of it was a measurement artifact -- the whole `subscribe()`->`onApplied` window under CDP network emulation holds the single response WebSocket frame for its emulated transfer time (confirmed against a no-emulation control leg: ~31-81ms unemulated vs. 3.7-12.4s emulated at the same row count/CPU profile), not server-side query evaluation or CPU contention. Once split by CDP frame timestamps, the isolated client decode/apply term is ~55 ms at 28k rows, throttled -- comfortably inside its 150 ms trigger, so D4's revisit is cleared this run. Not a blocker for the story itself (the deliverable was the measurement), but atlas batching is a real, structural risk for whichever story next builds the real boot path (FR144-146) |
