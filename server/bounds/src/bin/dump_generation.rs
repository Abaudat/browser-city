...
//! output at the fixed evidence seed. Run via `cargo run -p bounds --bin
...
use bounds::generation_evidence::{build_both, land_use_svg_path, streets_svg_path};
...
fn main() {
    let (land_use_svg, streets_svg) = build_both();
    write(land_use_svg_path(), &land_use_svg);
    write(streets_svg_path(), &streets_svg);
}
